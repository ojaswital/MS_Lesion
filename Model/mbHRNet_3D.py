...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv3d(3, 48, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm3d(48, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv3d(48, 48, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn2 = nn.BatchNorm3d(48, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        #self.layer1 = self._make_layer(Bottleneck, 16, 4)
        #self.stage2_cfg = extra['STAGE2']\
        self.stage2_cfg={'NUM_MODULES':1,'NUM_BRANCHES':2,'NUM_BLOCKS':[1,1],'NUM_CHANNELS':[48,96],'BLOCK':'BASIC','FUSE_METHOD':'SUM'}
        num_channels = self.stage2_cfg['NUM_CHANNELS']#[32, 64]
        block = blocks_dict[self.stage2_cfg['BLOCK']]#basic
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))
        ]
        self.transition1 = self._make_transition_layer([48], num_channels)#[32,64]
        self.stage2, pre_stage_channels = self._make_stage(
            self.stage2_cfg, num_channels)

        #self.stage3_cfg = extra['STAGE3']
        self.stage3_cfg = {'NUM_MODULES': 1, 'NUM_BRANCHES': 3, 'NUM_BLOCKS': [1,1,1], 'NUM_CHANNELS': [48,96,192],
        ...
        self.stage4_cfg = {'NUM_MODULES': 1, 'NUM_BRANCHES': 4, 'NUM_BLOCKS': [1,1,1,1], 'NUM_CHANNELS': [48,96,192,192],
                           'BLOCK': 'BASIC', 'FUSE_METHOD': 'SUM'}
