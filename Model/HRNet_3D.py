        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    '''
                    diffD = y.size()[2] - x1.size()[2]
                    diffX = y.size()[3] - x1.size()[3]
                    diffY = y.size()[4] - x1.size()[4]
...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.conv1 = nn.Conv3d(1, 16, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn1 = nn.BatchNorm3d(16, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv3d(16, 16, kernel_size=3, stride=1, padding=1,
                               bias=False)
        self.bn2 = nn.BatchNorm3d(16, momentum=BN_MOMENTUM)
        self.relu = nn.ReLU(inplace=True)
        # self.layer1 = self._make_layer(Bottleneck, 16, 4)
        #self.stage2_cfg = extra['STAGE2']\
        self.stage2_cfg={'NUM_MODULES':1,'NUM_BRANCHES':2,'NUM_BLOCKS':[1,1],'NUM_CHANNELS':[16,32],'BLOCK':'BASIC','FUSE_METHOD':'SUM'}
        num_channels = self.stage2_cfg['NUM_CHANNELS']#[32, 64]
        block = blocks_dict[self.stage2_cfg['BLOCK']]#basic
        num_channels = [
            num_channels[i] * block.expansion for i in range(len(num_channels))
        ]
        self.transition1 = self._make_transition_layer([16], num_channels)#[32,64]
        self.stage2, pre_stage_channels = self._make_stage(
            self.stage2_cfg, num_channels)

        #self.stage3_cfg = extra['STAGE3']
        self.stage3_cfg = {'NUM_MODULES': 1, 'NUM_BRANCHES': 3, 'NUM_BLOCKS': [1,1,1], 'NUM_CHANNELS': [16, 32, 64],
        ...
        self.stage4_cfg = {'NUM_MODULES': 1, 'NUM_BRANCHES': 4, 'NUM_BLOCKS': [1,1,1,1], 'NUM_CHANNELS': [16, 32, 64, 64],
                           'BLOCK': 'BASIC', 'FUSE_METHOD': 'SUM'}
    ...
    def _make_transition_layer(
        ...
        # print(nn.ModuleList(transition_layers))
...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.stage2_cfg={'NUM_MODULES':2,'NUM_BRANCHES':2,'NUM_BLOCKS':[1,1],'NUM_CHANNELS':[18,36],'BLOCK':'BASIC','FUSE_METHOD':'SUM'}
        ...
        self.stage3_cfg = {'NUM_MODULES': 3, 'NUM_BRANCHES': 3, 'NUM_BLOCKS': [2,2,2], 'NUM_CHANNELS': [18, 36, 72],
                           'BLOCK': 'BASIC', 'FUSE_METHOD': 'SUM'}
        num_channels = self.stage3_cfg['NUM_CHANNELS']
        block = blocks_dict[self.stage3_cfg['BLOCK']]
        num_channels = [
        ...
        self.stage4_cfg = {'NUM_MODULES': 2, 'NUM_BRANCHES': 4, 'NUM_BLOCKS': [2,2,2,2], 'NUM_CHANNELS': [18,36,72,144],
                           'BLOCK': 'BASIC', 'FUSE_METHOD': 'SUM'}
        num_channels = self.stage4_cfg['NUM_CHANNELS']
        block = blocks_dict[self.stage4_cfg['BLOCK']]
        num_channels = [
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv3d(
                    self.inplanes, planes * block.expansion,#64,256
                    kernel_size=1, stride=stride, bias=False
                ),
                nn.BatchNorm3d(planes * block.expansion, momentum=BN_MOMENTUM),
            )
